...
import static org.jetbrains.research.refactorinsight.processors.RefactoringType.CHANGE_ATTRIBUTE_TYPE;
import static org.jetbrains.research.refactorinsight.processors.RefactoringType.CHANGE_PARAMETER_TYPE;
import static org.jetbrains.research.refactorinsight.processors.RefactoringType.CHANGE_VARIABLE_TYPE;
import static org.jetbrains.research.refactorinsight.processors.RefactoringType.RENAME_ATTRIBUTE;
import static org.jetbrains.research.refactorinsight.processors.RefactoringType.RENAME_PARAMETER;
...
public class Utils {
    ...
    private static Predicate<RefactoringInfo> ofType(RefactoringType type) {
        return (r) -> r.getType().equals(type.getName());
    }
    ...
    public static boolean isStatementsEqualKotlin(
            @NotNull List<org.jetbrains.research.kotlinrminer.ide.decomposition.AbstractStatement> statementsBefore,
            @NotNull List<org.jetbrains.research.kotlinrminer.ide.decomposition.AbstractStatement> statementsAfter) {
        if (statementsBefore.size() == statementsAfter.size()) {
            boolean equal = true;
            for (int i = 0; i < statementsBefore.size() && equal; i++) {
                equal = statementsBefore.get(i).equalFragment(statementsAfter.get(i));
