...
public class RefactoringAction extends AnAction {
  ...
  public void actionPerformed(@NotNull AnActionEvent e) {
    ...
    final ProjectLevelVcsManagerImpl instance =
        (ProjectLevelVcsManagerImpl) ProjectLevelVcsManager.getInstance(currentProject);
    final VcsRoot gitRootPath =
    ...
    String branch = GitRepositoryManager.getInstance(currentProject).getRepositories().stream()
        .filter(x -> x.getRoot().equals(gitRootPath.getPath()))
    ...
    try {
      miner.detectAll(gitService.openRepository(currentProject.getBasePath()), branch,
