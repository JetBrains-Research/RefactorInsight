...
public class SampleDialogWrapper extends DialogWrapper {
  ...
  public SampleDialogWrapper(Project project, PsiElement before, PsiElement after) {
    ...
    final File file =
        new File("/home/matei/IdeaProjects/template/src/main/java/calculators/FoodCalculator.java");
  ...
  protected JComponent createCenterPanel() {
    ...
    DocumentContent oldContent =
        contentFactory.create(project, "Before Refactoring" + before.getText(), virtualFile);
    DocumentContent newContent =
        contentFactory.create(project, "After Refactoring" + after.getText(), virtualFile);
    SimpleDiffRequest request =
        new SimpleDiffRequest(null, oldContent, newContent, "Before", "After");
    ArrayList<Range> ranges = new ArrayList<>();
    ranges.add(new Range(4, 5, 6, 7));
    request.putUserData(DiffUserDataKeysEx.CUSTOM_DIFF_COMPUTER, getDiffComputer(ranges));

    DiffRequestPanel diffPanel =
        DiffManager.getInstance().createRequestPanel(project, getDisposable(), null);
  ...
  DiffUserDataKeysEx.DiffComputer getDiffComputer(@NotNull Collection<? extends Range> ranges) {
    return (text1, text2, policy, innerChanges, indicator) -> {
      InnerFragmentsPolicy fragmentsPolicy =
          innerChanges ? InnerFragmentsPolicy.WORDS : InnerFragmentsPolicy.NONE;
      ...
      for (Range range : ranges) {
        result.addAll(comparisonManager
            .compareLinesInner(range, text1, text2, offsets1, offsets2, policy, fragmentsPolicy,
                indicator));
      }
      return result;
    };
